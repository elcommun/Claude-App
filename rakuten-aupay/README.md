...
- **バリエーション在庫**: SKUごとに stock.csv 行を生成（横軸 choicesStockHorizontal、`&`→`/` 変換）。Codeは選択肢名末尾の（）内のSKU番号を抽出（例: 「flower（JYX-001）」→ Name「flower（JYX-001）」／Code「JYX-001」）。choicesStockShippingDayIdは空欄で出力
