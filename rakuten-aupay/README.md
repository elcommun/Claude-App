...
- **説明文**: キャッチコピー→description、スマホ用説明文→descriptionForSP、PC用販売説明文→descriptionForPC。改行除去、`image.rakuten.co.jp/elcommun/cabinet` → `image.wowma.jp/69585078/elcommun/cabinet` 書き換え、商品ページリンクは `/u/69585078/c/品番` に変換、楽天カテゴリリンク（`/c/数字/`）は内蔵の対応表（209件）で `/user/69585078/shopcategory/ID` に変換。対応が無いカテゴリリンク・変換できない楽天リンクはアンカー解除して警告
- **カテゴリ**: ジャンルID→auPAYカテゴリIDは実績データから抽出した122件のマッピングを内蔵（未対応は警告）
- **店舗内カテゴリ**: 楽天カテゴリURL→auPAY店舗内カテゴリ（shopCategoryName / shopCategoryId）の対応表（209件、ユーザー提供CSVから生成）で照合し、auPAY側の正式カテゴリ名で出力。対応が無いカテゴリは除外して警告一覧に表示。「セール商品」は黙って除外、親階層の重複も除外、最大10件
